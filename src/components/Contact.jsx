    ...
    <section id="contact" className="py-20 px-4 md:py-28 transition-colors duration-300">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          />
          <p className="text-slate-700 dark:text-slate-200 mt-6 text-sm max-w-xl mx-auto leading-relaxed">
        ...
        >
          {contactItems.map((item) => {
            ...
            const cardContent = (
              <>
                ...
                <div className="text-left">
                  <h4 className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">{item.label}</h4>
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 mt-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors break-all">
                    {item.value}
                  </p>
                </div>
              </>
            ...
            const containerClassName = `group flex items-center gap-4 p-5 rounded-2xl bg-white/90 dark:bg-slate-900/80 backdrop-blur-sm border border-slate-200/70 dark:border-slate-800/80 shadow-md hover:shadow-lg transition-all duration-300 ${item.href ? 'cursor-pointer' : 'cursor-default'} ${isLinkedIn ? 'sm:col-span-2 lg:col-span-1' : ''}`;
